"""COMMAND : .runs , .metoo , .pro, .chase, .abuse, .belo """
...
async def _(event):
    ...
    if reply_to:
        await reply_to.reply(random.choice(LOGICAL))
        await event.delete()
    else:       
        await edit_or_reply(event, event.from_id, random.choice(LOGICAL))
...
async def _(event):
    ...
    if reply_to:
      await event.delete()
      await reply_to.reply(random.choice(ABUSE_STRINGS))
    else:
...
async def _(event):
    ...
    if reply_to:
      await event.delete()
      await reply_to.reply(random.choice(CHASE_STR))
    else:
